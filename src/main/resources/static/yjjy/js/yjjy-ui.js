...
(function ($) {
    $.extend({
        ...
        uploadFiles: {
            _option: {},
            _params: {},
            init: function (fileUrlId, fileNameId, flag, num) {
                var url = ctx + "system/uploadFile/uploadFiles/" + flag;
                var options = {
                    title: '上传附件',
                    width: "500",
                    height: "500",
                    ...
                    callBack: function (index, layero) {
                        layero.find('iframe')[0].contentWindow.uploadFiles($('#' + fileUrlId), $('#' + fileNameId), layer, index);
                    }
                };
        ...
        orderUploadFiles: {
            _option: {},
            _params: {},
            init: function (fileUrlId, fileNameId, as) {
                var url = ctx + "system/uploadFile/orderUploadFiles";
                var options = {
                    title: '上传附件',
                    width: "500",
                    height: "350",
                    url: url,
                    callBack: function (index, layero) {
                        layero.find('iframe')[0].contentWindow.uploadFiles($('#' + fileUrlId), $('#' + fileNameId), $('#' + as), layer, index);
                    }
                };
                $.modal.openOptions(options);
            }
        ...
        table: {
            ...
            _params: {},
            // 初始化表格参数
            init: function (options, myTable) {
                if ($.common.isEmpty(myTable)) {
                    myTable = 'bootstrap-table';
                }
                var bootstrap_table = $('#' + myTable);
                ...
                var table = bootstrap_table.bootstrapTable({
                    url: options.url,                                   // 请求后台的URL（*）
                    uniqueId: options.uniqueId,
                    contentType: "application/x-www-form-urlencoded",   // 编码类型
                    clickToSelect: true,                                //是否启用点击选中行
                    singleSelect: $.common.isEmpty(options.singleSelect) ? false : options.singleSelect,
                    method: 'post',                                     // 请求方式（*）
                    cache: false,                                       // 是否使用缓存
                    ...
                    onLoadSuccess: options.onLoadSuccess,               // 当所有数据被加载时触发（*）
            ...
            // },
            // 搜索-默认第一个form
            search: function (formId, myTable) {
                var params = $.table.gainParams(formId, myTable);
                ...
                var bootstrap_table = $('#' + myTable);
                // $("#bootstrap-table").bootstrapTable('refresh', params);
                bootstrap_table.bootstrapTable('refreshOptions', {pageNumber: num});
            },
            // 搜索
            gainParams: function (formId, myTable) {
                var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
                if ($.common.isEmpty(myTable)) {
                    myTable = 'bootstrap-table';
                }
                var bootstrap_table = $('#' + myTable);
                var params = bootstrap_table.bootstrapTable('getOptions');
                params.queryParams = function (params) {
                    var search = {};
                    $.each($("#" + currentId).serializeArray(), function (i, field) {
                        search[field.name] = field.value;
            ...
            // 查询表格指定列值
            selectColumns: function (column, myTable) {
                if ($.common.isEmpty(myTable)) {
                    myTable = 'bootstrap-table';
                }
                var bootstrap_table = $('#' + myTable);
                return $.map(bootstrap_table.bootstrapTable('getSelections'), function (row) {
                    return row[column];
                });
            },
            // 查询表格首列值
            selectFirstColumns: function (myTable) {
                if ($.common.isEmpty(myTable)) {
                    myTable = 'bootstrap-table';
                }
                var bootstrap_table = $('#' + myTable);
            ...
            // 回显数据字典
            selectDictLabelStyle: function (datas, value) {
                var actions = [];
        ...
        form: {
            ...
            reset: function (formId) {
                ...
                $("#deptId").val('');
                $("#postId").val('');
                $("#select2-postId-container").text('');
                // $("#employeeLevelDictCode").val('');
                $("#select2-employeeLevelDictCode-container").text('');
            },
            // 获取选中复选框项
        ...
        modal: {
            ...
            // 弹出层指定宽度
            open: function (title, url, width, height, flag, btn) {
                //如果是移动端，就使用自适应大小弹窗
            ...
            openhtml: function (title, url, width, height) {
                layer.open({
                    type: 2,
                    area: [width + 'px', height + 'px'],
                    fix: false,
        ...
        operate: {
            ...
            muiscAll: function() {
                ...
                $.modal.confirm("确认全区进行播放吗?", function() {
                    var url = $.table._option.startUrl;
                    var data = { "id": "0" };
                    $.operate.submit(url, "post", "json",data);
                });
            },
            ...
            muiscStop: function() {
                ...
                $.modal.confirm("确认全区停止播放吗?", function() {
                    var url = $.table._option.stopUrl;
                    var data = { "id": "1" };
                    $.operate.submit(url, "post", "json", data);
                });
            },
            ...
            // 添加信息
            addHaveParam: function (width, height, flag) {
                $.modal.open("添加" + $.table._option.modalName, $.table._option.createUrl, width, height, flag);
            },
            // 修改信息
            edit: function (id) {
                var url = "/404.html";
            ...
            editDetails: function (type, id) {
                layer.open({
                    ...
                    title: "修改记录",
                    area: [1200 + 'px', 800 + 'px'],
                    content: ctx + "edit/editRecord?sourceType=" + type + "&sourceId=" + id,
                    success: function (layer) {
                        layer[0].childNodes[3].childNodes[0].attributes[0].value = 'layui-layer-btn1';
                    },
                    btn1: function (index) {
        ...
        layUi: {
            upload: {
                init: function (id, url, accept, data, type) {
                    var uploadInst;
                        ...
                        uploadInst = layui.upload.render({
                            ...
                            before: function (obj) {
                                ...
                                if (res.code == 0) {
                                    ...
                                    $.modal.msgError('导入文件失败！');
                                    if (type) {
                                        var ev = document.createEvent("MouseEvents");
                                        ev.initMouseEvent("click", true, false, window, 0, 0, 0, 0, 0, false, false, false, false, 0, null);
                                        var urlObject = window.URL || window.webkitURL || window;
                                        var save_link = document.createElementNS("http://www.w3.org/1999/xhtml", "a")
                                        save_link.href = urlObject.createObjectURL(new Blob([res.msg]));
                                        save_link.download = type;
